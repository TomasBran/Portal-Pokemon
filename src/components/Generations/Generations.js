...
const Generations = ({ getGenerations, resetGame, padding = 4 }) => {
	...
	return (
		<div>
			...
			<div
				className={`p-4 w-[36vw] fixed right-2 bottom-2 bg-white rounded-lg border-2 border-gray-600 flex flex-col gap-6 items-center text-white ${!showGenerationsContainer && 'hidden'}`}>
				<div className='flex flex-wrap gap-6 justify-center'>
					{currentGenerations.map((element, index) => (
						<div
							key={index}
							className={`w-3/12 py-2 rounded-lg cursor-pointer active:scale-95 ${element === true ? 'bg-green-600 hover:bg-green-500 active:bg-green-400' : 'bg-red-600 hover:bg-red-500 active:bg-red-400'}`}
							onClick={() => toggleGeneration(index)}>
							<span>{index + 1}° Generación</span>
						</div>
					))}
				</div>
